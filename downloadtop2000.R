...
while(TRUE)
{
  ...
  for(i in 1:nrow(channelobj))
  {
    ...
    cmd<-paste0("/home/root/automate-save-page-as/save_page_as https://www.youtube.com/channel/",channelobj[i,]$channelid,"/videos -b firefox -d /mnt/webdownload/",channelobj[i,]$channelid," --pagedown ",channelobj[i,]$pagedown," --savewaittime ",5)
    system(cmd)
    cat(paste(cmd,"\n"))
