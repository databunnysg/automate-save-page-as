...
channel<-data.frame(channelid="UCPcF3KTqhD67ADkukx_OeDg",force=TRUE,pagedown=100,save_wait_time=30,stringsAsFactors = FALSE)
...
while(TRUE)
{
  ...
  for(i in 1:nrow(channelobj))
  {
    ...
    cmd<-paste0("/home/root/automate-save-page-as/save_page_as https://www.youtube.com/channel/",channelobj[i,]$channelid,"/videos -b firefox -d /mnt/webdownload/",channelobj[i,]$channelid," --pagedown ",channelobj[i,]$pagedown," --save_wait_time ",channelobj[i,]$save_wait_time)
    system(cmd)
    cat(paste(cmd,"\n"))
    #Sys.sleep(2)
  }
